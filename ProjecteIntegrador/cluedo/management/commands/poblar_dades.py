...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        habitacions = [
            ...
            ('Sala de Billar', 'Amb una taula de billar al centre i trofeus a les parets'),
        ...
        for nom, desc in personatges:
            _, created = Personatge.objects.get_or_create(nom=nom, defaults={'descripcio': desc})
            if created:
                creats += 1
                self.stdout.write(f'  Creat personatge: {nom}')

        for nom, desc in armes:
            _, created = Arma.objects.get_or_create(nom=nom, defaults={'descripcio': desc})
            if created:
                creats += 1
                self.stdout.write(f'  Creada arma: {nom}')

        for nom, desc in habitacions:
            _, created = Habitacio.objects.get_or_create(nom=nom, defaults={'descripcio': desc})
            if created:
                creats += 1
