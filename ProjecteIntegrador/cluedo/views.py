...
def register_view(request):
    ...
    if len(password) < 6:
        return JsonResponse(
            {"error": "La contrasenya ha de tenir almenys 6 caràcters"}, status=400
        )
...
def partida_activa_view(request):
    ...
    try:
        solucio = Solucio.objects.filter(usuari=request.user, resolta=False).latest(
            "creada_en"
        )
        return JsonResponse(
            {
...
def nova_partida_view(request):
    ...
    if not personatges or not armes or not habitacions:
        return JsonResponse(
            {"error": "No hi ha dades al joc. Executa poblar_dades."}, status=500
        )
...
def historial_view(request):
    ...
    solucions = (
        Solucio.objects.filter(usuari=request.user)
